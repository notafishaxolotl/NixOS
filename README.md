...
- And make sure to replace my username (`acito`) with your own in: [`configuration.nix`](nixos/configuration.nix),  [`vm.nix`](nixos/vm.nix),  [`flake.nix`](.dotfiles/flake.nix),  [`home.nix`](.dotfiles/home.nix). Otherwise your build will just fail (learnt from experience).
